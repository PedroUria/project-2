...
Now, let us encode `certificate`, which follows this [system](https://www.wikiwand.com/en/Motion_Picture_Association_of_America_film_rating_system) and this [one](https://www.wikiwand.com/en/TV_parental_guidelines_(US)#/TV-MA). That is, `certificate` is an ordinal variable and needs to be encoded as such (preserving the order). Well... actually, even if there is some order in it, we would not know how to interpret this order, and it could also be argued that there is no order to begin with, so we will turn it into a factor instead (which will then be one-hot-encoded by R when running Logistic Regression)

Before doing so, we need to treat any missing values
```{r}
...
and although it did win an Oscar and 5 other [awards](https://www.imdb.com/title/tt5278466/awards?ref_=tt_awd), TODO: our only option is to drop it
```{r}
oscars  = oscars[!(oscars$certificate=="TV-MA"),]
```

Finally, let us turn `certificate` into a factor
```{r}
oscars$certificate = factor(oscars$certificate)
```
...
TODO: Lol, the movie I dropped above on certificate was the same one as the one below, so I  deleted the same process for `genre_documentary`.
...
# We need to split the train data into training and testing so that
# we don't use our test data for hypertuning mtry
set.seed(42) 
train.index <- createDataPartition(oscars_train.renamed$Oscars_won_some, p = .7, list = FALSE)
train <- oscars_train.renamed[ train.index,]
test  <- oscars_test.renamed[-train.index,]
y_testt <- test$Oscars_won_some
...
TODO: Well, now it seems it does not. The only changes are certificate as factor (the standardizing on the cross-val loop doesn't affect this). We either keep certificate as we encoded it before, or we try to hypertune mtry using cross-val?? Otherwise I don't see how we can get RF to perform better than LR and then have that nice conclusion about showing the importance of doing cross-val.
