...
table(oscars$year)
```
about 70 movies, but for 2017 we only have 30. We are thus not going to use this year...
```{r}
# Lied about not doing yet any preprocessing...
oscars = subset(oscars, year != 2017)
```
...
The next feature that needs some manipulation is `gross`. As 1 dollar in 2000 is [worth](http://www.in2013dollars.com/2000-dollars-in-2016?amount=1) 1.39 dollars in 2016, we need to account for this inflation. To do this, we will use (TODO: Zach, I think you are better off explaining this haha)
...
TODO: Use `release_date.month` to create `season`.
