...
We will use a dataset that [BigMl](https://bigml.com/) [put together](https://bigml.com/user/academy_awards/gallery/dataset/5a94302592fb565ed400103b) and used to train a deep neural network to get [perfect predictions](https://blog.bigml.com/2018/03/01/predicting-the-2018-oscar-winners/) for the 8 categories they targeted for 2018 using data on oscar eligible films from the years 2000-2017. However, this does not mean their model is perfect, as the prediction accuracy on their training set was not 100% when they tested their model on years prior to 2018.

Note: the oscars have 24 categories, and this dataset only recorded whether a film won 8 of those awards.

Although our data came from a machine learning company, it still needed a plethora of pre-processing before random forest or logistic regression models could be trained on it.
...
*Note: The code for attempting approach 1 is available in the Rmd file, but will not run for the html file. To run it use the function `certificate_factor` by uncommenting the line that executes it.*
...
plot_top_5_imp = function(VI_F2){
  ...
  top5[1,2]
  # https://freakonometrics.hypotheses.org/19835
  barplot(top5[,1],names= top5[,2],cex.names=.4,
...
The 5 most important features for predicting our target, using our random forest model with 500 trees (in descending order) are the number of screen actor's guild awards a movie won, the total number of non-oscar awards a movie won, the number of golden globes a movie won, the total number of non-oscar awards a movie won and the number of directors guild awards a movie won.

Obviously, the number of non-oscar awards won by a movie from a specific organization is going to be correlated with the total number of non-oscar awards won by that movie (same with nominations), so interpreting individual feature importances is not entirely clear, but since we grew 500 trees that used roughly half of the variables at each split, we have reason to believe that at least some trees were grown without using the aggregated variables in the initial splits (giving the plethora of individual non-oscar award variables a chance to yield the best initial split and yield the highest decrease in the gini index.)
...
The most important features (in order) for predicting our target in our random forest model with 1000 trees are exactly the same as for the random forest model with 500 trees. We are not sure whether this means the random forest with 500 trees was able to separate the aggregated and individual non-oscar awards well enough to fix our collinearity problem, or if we simply have not run a random forest with enough trees.
...
The model that consistently had the most predictive capability was a Logistic Regression Model using variables selected via Stepwise Forward Selection (number 6 in the table below). We base this statement on the fact that this model’s cross-validated accuracies have the highest mean and the lowest standard deviation. The model also provides the highest AUC.  

| Num | Model Type           | Var Selection           |  Num Vars | CV Accuracy | CV Stdev | AUC    |
|-----|----------------------|-------------------------|-----------|-------------|----------|--------|
| 1.  | Logistic Reg         | None                    | 81        | N/A         | N/A      | N/A    |
| 2.  | Logistic Reg         | Manual                  | 12        | 0.9458      | 0.0130   | 0.9548 |
| 3.  | Logistic Reg         | Manual & Forward (BIC)  | 4         | 0.9529      | 0.0094   | 0.9582 |
| 4.  | Random Forest        | 35 random per split     | 81        | 0.9609      | 0.0122   | 0.9740 |
| 5.  | Random Forest (1000) | 14 random per split     | 81        | 0.9636      | 0.0100   | 0.9754 |
| 6.  | Logistic Reg         | Forward (BIC)           | 7         | 0.9689      | 0.0040   | 0.9815 |

Prior to using the cross-validated data to compare the models, it appeared that Random Forest with 500 trees was the best predictive model we had trained, but as we can see from the table above, this is not the case. This highlights the importance of using cross validation when evaluating and comparing different models, to ensure we have a more generalizable model that is not overfitting the training data.

It is also worth mentioning, that even if model 6 fails its multicollinearity assumption, that is only problematic when using our model to make inference regarding the relationships between our features and the target variable, such as identifying the most relevant features. Since we are trying to create a model that is useful only in predicting the oscars, we are not overly concerned by the multicollinearity in this case.   

To sum up, we were able to train a model with a decent predictive power relative to the no information rate in our model. 
