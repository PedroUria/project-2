...
```{r}
# Trains the model
oscars_train_logit <-  glm(Oscars_won_some~certificate + duration+rate+metascore+votes+user_reviews+critic_reviews+popularity+awards_wins+awards_nominations+sc.gross+seasons,
...
score <- sum(ifelse(prob_test_pred > 0.5,"Yes","No")==y_test)/length(y_test)
score



oscars_train_logit2 <-  glm(Oscars_won_some~.,
                family = binomial(link = "logit"), data = select(oscars_train, matches("genre_.*"), Oscars_won_some))
oscars_train_logit2 <-  glm(Oscars_won_some~.,
                family = binomial(link = "logit"), data = select(oscars_train, matches("_nominated$"), awards_wins, Oscars_won_some))
#Prints out warnings
#Warning messages:
#1: glm.fit: algorithm did not converge 
#2: glm.fit: fitted probabilities numerically 0 or 1 occurred 
summary(oscars_train_logit2)
# lol, this is maybe because of the missingness that hasn't been dealt with yet
#sapply(oscars, function(x) sum(is.na(x)))
# well, actually we don't have that many... because we dropped the categories features
# for this try. popularity does have a significant number, but even so, not so sure this
...
# Tests the model
prob_test_pred2 <- predict.glm(oscars_train_logit2,oscars_test,type='response')
score <- sum(ifelse(prob_test_pred2 > 0.5,"Yes","No")==y_test)/length(y_test)
score
# ROC Curve
...
glm.null <-  glm(Oscars_won_some~1,
                family = binomial(link = "logit"), data = oscars_train)
glm.full <- glm(Oscars_won_some~.,
                family = binomial(link = "logit"), data = oscars_train)
glm.final <- step(glm.null, scope = formula(glm.full), direction = "forward", k = log(nrow(oscars_train)))

glm.final.test <- predict.glm(glm.final,oscars_test,type='response')
score <- sum(ifelse(glm.final.test > 0.999,"Yes","No")==y_test)/length(y_test)
score

# ROC Curve
prob=plogis(predict.glm(glm.final, type = c("response")))
#head(prob)
h <- roc(Oscars_won_some~prob, data=oscars_train)
plot(h)
# Area Under the Curve
...
confusionMatrix(factor(ifelse(glm.final.test > 0.5,"Yes","No")),y_test)
confusionMatrix(factor(ifelse(glm.final$fitted.values > 0.5,"Yes","No")), oscars_train$Oscars_won_some)
```
