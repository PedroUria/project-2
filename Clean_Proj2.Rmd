...
First we'll build a Multiple Logistic Regression model, then we'll attempt to improve upon it using step-wise forward variable selection. Apart from using our train and test sets to evaluate our models, we will also use 3-fold stratified cross-validation to test the variance of our predictive accuracy. We chose stratified cross-validation to ensure the proportion of class labels is maintained in each of the splits. We chose 3-folds instead of the standard 10-folds, because if we were to use smaller splits we'd risk having too few degrees of freedom to train the model, given the scarcity of the oscar winners.
...
# I WAS GOING TO MAKE A FUNCTION OUT OF THIS BUT SEEMS IMPOSSIBLE
# The only parameter would be the variable names to pass onto glm
# but I don't know how to make it work and not gonna waste my time
# on this for now, maybe by Friday we can find a way to do it, but
# we don't need it for the presentation
...
for(i in list_){
  ...
  # Trains the model 
  logreg.train <- glm(Oscars_won_some~certificate + duration+rate+metascore+votes+user_reviews+critic_reviews+popularity+awards_wins+awards_nominations+sc.gross+seasons,
                family = binomial(link = "logit"), data = cv.train)
  # Tests the model
...
for(i in list_){
  ...
  cv.test = oscars[test_index, ]
  # Trains the model 
  logreg.train <- glm(Oscars_won_some~awards_wins+Screen_Actors_Guild_won+
                        Golden_Globes_won+genre_Drama+Writers_Guild_won+
...
for(i in list_){
  ...
  cv.test = oscars[test_index, ]
  # Trains the model 
  logreg.train <- glm(Oscars_won_some~awards_wins+genre_Drama+genre_Biography+certificate, family = binomial(link = "logit"),
      data = cv.train)
  # Tests the model
  glm.final.test <- predict.glm(logreg.train,cv.test,type='response')
  score <- sum(ifelse(glm.final.test > 0.5,"Yes","No")
...
# Its output is:
#[1] 19 22 23 27 28 31 33 35 36 39 40 41 42 44 45 47 49 50 51 52 53 55 56 57 58 59 60 61 62 63
#[31] 64 66 67 68 69 70 71 72 73 74 75 76 77 78 79 80

# Well... it seems a lot of different mtry give the same best score
# Let's try it with 35
...
for(i in list_){
  ...
  test_index = folds[[i]]
  # Splits between training and testing
  cv.train = oscarss[train_index, ]
  cv.test = oscarss[test_index, ]
  # Trains the model 
  cv.rf <- randomForest(formula=Oscars_won_some~.,
               importance=TRUE, 
               proximity=TRUE,
               mtry=35, 
               data=cv.train)
  # Tests the model
  y_test_pred <- predict(cv.rf,cv.test,
                         type='response')
  score <-sum(y_test_pred==cv.test$Oscars_won_some)/
    length(cv.test$Oscars_won_some)
  # Appends the score to the list 
  scores = c(scores, score)
}
